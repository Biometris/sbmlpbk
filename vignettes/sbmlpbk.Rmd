...
Running PBK model simulations with various dosing scenarios requires construction of `deSolve` \code{events} patterns. As PBK models may differ in time resolution (e.g., hours/days) and dosing amounts (e.g., mg, ug, moles) creation of dosing scenarios, running simulations, and evaluating outputs requires alignment of the model's time resolution and dosing/output amounts with the desired resolutions. The methods \code{create_dosing_events} and \code{create_times_seq} are provided to assist with this. The method \code{create_dosing_events} creates `deSolve`-compatible events from a specified dosing scenario in a user-specified time unit and amounts unit. Likewise, the method \code{create_times_seq} creates a \code{deSolve} compliant \code{times} sequence, specifying the times for which simulation output is wanted.
